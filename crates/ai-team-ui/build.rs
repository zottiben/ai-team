...
//!
//! Nothing it hands cargo names this checkout. Worktrees of one repository that share a
//! `CARGO_TARGET_DIR` share this script's output as well, because cargo names a path
//! package's build directory by where it sits in its workspace rather than by where the
//! workspace is. An absolute path in that output is another worktree's the moment one
//! builds after it: its binary embedded the other's window. So the table includes each
//! file relative to the crate being compiled, and the paths cargo watches are relative
//! to the package - which is how cargo resolves them for whichever checkout is building.

use std::path::{Path, PathBuf};
use std::process::Command;
use std::time::SystemTime;

/// The frontend, from this crate's directory. Written relative everywhere it leaves this
/// script, never resolved: see the module comment.
const UI: &str = "../../ui";
...
fn main() {
    ...
    for path in [
        ...
        println!("cargo:rerun-if-changed={UI}/{path}");
    }
...
/// Every file under `dist`, as (url path, mime).
fn collect(dist: &Path) -> std::io::Result<Vec<(String, &'static str)>> {
...
fn render(files: &[(String, &'static str)]) -> String {
    ...
    for (path, mime) in files {
        // Resolved by rustc against the crate it is compiling, so the bytes are always this
        // checkout's own, and rustc reports the file for cargo to watch like any source.
        let _ = writeln!(
            out,
            "    ({path:?}, {mime:?}, include_bytes!(concat!(env!(\"CARGO_MANIFEST_DIR\"), {:?}))),",
            format!("/{UI}/dist/{path}")
        );
    }
    out.push_str("];\n");
