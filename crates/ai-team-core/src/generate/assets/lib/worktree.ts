...
//
// Every authored tool resolves through here, so the guard is written once and audited
// once. ai-team starts one eve process per leased worktree (D10), so the root is a
// property of the process rather than of a request the model could influence.
...
export function worktreeRoot(): string {
  ...
  if (!raw || raw.trim() === "") {
    throw new Error(
      `${WORKTREE_ENV} is not set. ai-team starts one eve process per leased worktree; ` +
